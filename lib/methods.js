...
import { Chats, Messages, Tasks, Diaries, Stresslevels } from '../lib/collections';
...
Meteor.methods({
  ...
  addStresslevel(stressvalue, status){
    if (!this.userId) {
      throw new Meteor.Error('not-logged-in',
        'Must be logged to remove a chat.');
    }
    ...
    const stresslevel = Stresslevels.findOne({createdAt : {'$gte' : d, '$lt' : end}, userId : this.userId})
    ...
    if (!stresslevel) {
        ...
        const stressId = Stresslevels.insert(stress);
        return stressId;
    }else{
        return Stresslevels.update(stresslevel._id, { $set: { 'value': stressvalue, 'status' : status, 'createdAt' : new Date()}});
    }
  },
});